...
exports.getSchedules = async (req, res, next) => {
	try {
		const [ rows, fields ] = await db.execute('SELECT name as teacherName, date, timing, batch, schedules.id FROM schedules INNER JOIN teachers ON teachers.id = schedules.teacherId');
...
exports.createSchedule = async (req, res, next) => {
	...
	try {
		let [ scs ] = await db.execute('SELECT * FROM schedules');
		for (let el of scs) {
		...
		let [ teachers ] = await db.execute('SELECT id FROM teachers WHERE name = ?', [ data.teacherName ]);
		await db.execute(
...
exports.removeSchedule = async (req, res, next) => {
	...
	console.log(id);
	try {
		let [rs] = await db.execute('SELECT COUNT(batch) as count FROM schedules  WHERE id = ?', [id]);
		if(rs[0].count == 0){
...
exports.updateSchedule = async (req, res, next) => {
	...
	try {
		...
		let [ teachers ] = await db.execute('SELECT id FROM teachers WHERE name = ?', [ data.teacherName ]);
		await db.execute(`UPDATE schedules SET timing = ?, date = ?, teacherId = ? WHERE id = ?`, [
			data.timing,
			data.date,
			teachers[0].id,
			id
...
exports.getTeacherSchedules = async(req, res, next) => {
	...
	try{
		let [teacher] = await db.execute('SELECT id FROM teachers WHERE name = ?',[name]);
		let[rows] = await db.execute('SELECT * FROM schedules WHERE teacherId = ?',[teacher[0].id]);
		res.status(200).json({
			message:"Success",
			data:rows
		})
